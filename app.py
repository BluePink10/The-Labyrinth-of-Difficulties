...
3 игрока в комнате: 1 вор + 2 мирных жителя.
...
class Room:
    ...
    async def try_start_game(self):
        ...
        for p in self.players.values():
            await self.send_to(p, {
                ...
                "world": {"tile": TILE, "grid_w": GRID_W, "grid_h": GRID_H, "num_levels": NUM_LEVELS},
                "duration": GAME_DURATION,
            })

    async def maybe_begin_play(self):
        if self.state == "ROLE" and all(p.ready for p in self.players.values()) and len(self.players) == 3:
            self.state = "PLAY"
    ...
    async def maybe_finish_vote(self):
        if self.state != "VOTE":
            return
        if not all(p.vote for p in self.players.values()):
            return
...
async def ws_endpoint(websocket: WebSocket, room_code: str):
    ...
    try:
        ...
        async with room.lock:
            if room.state != "LOBBY" or len(room.players) >= 3:
            ...
            avatar = msg.get("avatar") or random.choice(AVATARS)
            player = Player(pid, name, avatar, websocket)
            room.players[pid] = player
            if room.host_id is None:
                room.host_id = pid

            await websocket.send_text(json.dumps({"type": "joined", "you": pid, "host": room.host_id == pid}))
